...
def check_song_expiry(user_id: str, playlist_id: int):
    response = {}
    print(f"Checking song expiry for user {user_id} and playlist {playlist_id}")

    # Fetch the tracked playlist
    tracked_playlist_result = supabase.table('Tracked Playlists').select('*').eq('id', playlist_id).eq('user_id', user_id).single().execute()
    ...
    user_settings_result = supabase.table('User Settings').select('*').eq('user_id', user_id).single().execute()

    if not user_settings_result or not user_settings_result.data:
        print(f"Error fetching user settings for user {user_id}: {user_settings_result}. Task ended.")
        return {
            "message": f"Error fetching user settings for user {user_id}: {user_settings_result}. Task ended."
        }
    
    user_settings = UserSettings(**user_settings_result.data)

    if user_settings.playlist_persistence == "forever":
        print(f"Song persistence is set to forever. Task ended.")
        return {
            "message": f"Song persistence is set to forever. Task ended."
        }
    
    # Fetch the deleted songs
    deleted_songs_result = supabase.table('Deleted Songs').select('*').eq('user_id', user_id).eq('playlist_id', playlist_id).eq('active', True).execute()

    if not deleted_songs_result or not deleted_songs_result.data:
        print(f"Error fetching deleted songs for user {user_id}: {deleted_songs_result}. Task ended.")
        return {
            "message": f"Error fetching deleted songs for user {user_id}: {deleted_songs_result}. Task ended."
        }
    ...
    if len(songs_to_expire) > 0:
        if user_settings.remove_from_playlist and tracked_playlist.removed_playlist_id:
            ...
            spotify_service.remove_tracks_from_playlist(tracked_playlist.removed_playlist_id, removed_track_ids)
        ...
        for song in songs_to_expire:
            supabase.table('Deleted Songs').update({'active': False}).eq('id', song.id).execute()
